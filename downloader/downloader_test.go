...
func TestDownload(t *testing.T) {
	...
	imageContentRequest := downloader.ImageContentRequest{
		Url:       "http://localhost:8001/package.txt",
		RequestId: 1,
	}

	downloadedFile, err := downloadInstance.Download(context.Background(), imageContentRequest)
	if err != nil {
...
func TestConcurrentDownloads(t *testing.T) {
	...
	for i := 0; i < numDownloads; i++ {
		...
		go func(i int) {
			...
			imageContentRequest := downloader.ImageContentRequest{
				Url:       "http://localhost:8001/" + fmt.Sprintf(fileNamePattern, i),
				RequestId: 1,
			}

			_, err := downloadInstance.Download(context.Background(), imageContentRequest)
			if err != nil {
