...
type Downloader interface {
	Download(context.Context, downloader.ImageContentRequest) (fileName string, err error)
}
	...
	for {
		...
		default:
			...
			if request, ok := v.isImageContentRequest(buffer); ok {
				go func() {
					v.download(downloader.ImageContentRequest{
						RequestId: request.ImageContentRequest.RequestId,
						Url:       request.ImageContentRequest.Url,
...
func (v *Vchan) download(imageRequest downloader.ImageContentRequest, contentType string) {
	fileName, err := v.downloadManager.Download(v.ctx, imageRequest)
	if err != nil {
		log.Errorf("Failed to download image content: %v", aoserrors.Wrap(err))

		if err := v.sendFailedImageContentResponse(imageRequest.RequestId, err); err != nil {
	...
	imageContentInfo, imagesContent, err := v.getImageContent(fileName, contentType, imageRequest.RequestId)
	if err != nil {
